...
// register and their lazily allocated ELF TLS blocks. When the thread exits,
// the destructors run first and the blocks are freed after, so a destructor
// still sees its thread-local storage.
struct ThreadTls {
    ...
    // The pending dlerror() message of the thread. take() consumes it, and the
    // returned text stays valid until the next take, as dlerror() requires.
